...
type User struct {
	...
	AdminScopes      *AdminScopesPayload        `json:"admin_scopes,omitempty" gorm:"-:all"`
}
...
func GetAllUsers(pageInfo *common.PageInfo, scope DataScope, sortOptions ...UserSortOptions) (users []*User, total int64, err error) {
	// Start transaction
	tx := DB.Begin()
	if tx.Error != nil {
	...
	countQuery := ApplyIDScope(tx.Unscoped().Model(&User{}), "id", scope)
	...
	err = order.Apply(ApplyIDScope(tx.Unscoped(), "id", scope)).Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password", "access_token").Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, scope DataScope, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
	...
	query = ApplyIDScope(query, "id", scope)
...
func (user *User) Insert(inviterId int, requestIP ...string) error {
	...
	user.finishInsert(inviterId, firstExtraString(requestIP))
	return nil
}

func (user *User) finishInsert(inviterId int, ip string) {
	// 用户创建成功后，根据角色初始化边栏配置
	...
	if common.QuotaForNewUser > 0 {
		RecordLog(user.Id, LogTypeSystem, fmt.Sprintf("新用户注册赠送 %s", logger.LogQuota(common.QuotaForNewUser)), ip)
	}
	if inviterId != 0 && operation_setting.IsPaymentComplianceConfirmed() {
		if common.QuotaForInvitee > 0 {
			_ = IncreaseUserQuota(user.Id, common.QuotaForInvitee, true)
			RecordLog(user.Id, LogTypeSystem, fmt.Sprintf("使用邀请码赠送 %s", logger.LogQuota(common.QuotaForInvitee)), ip)
		}
		if common.QuotaForInviter > 0 {
			//_ = IncreaseUserQuota(inviterId, common.QuotaForInviter)
			RecordLog(inviterId, LogTypeSystem, fmt.Sprintf("邀请用户赠送 %s", logger.LogQuota(common.QuotaForInviter)), ip)
			_ = inviteUser(inviterId)
		}
	}
}

func (user *User) FinishInsert(inviterId int, requestIP ...string) {
	user.finishInsert(inviterId, firstExtraString(requestIP))
}
...
func (user *User) FinalizeOAuthUserCreation(inviterId int, requestIP ...string) {
	// 用户创建成功后，根据角色初始化边栏配置
	var createdUser User
