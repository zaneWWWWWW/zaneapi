...
var lastQuotaDataExportAt atomic.Int64

func GetLastQuotaDataExportAt() int64 {
	return lastQuotaDataExportAt.Load()
}
...
func SaveQuotaDataCache() error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		for _, quotaData := range CacheQuotaData {
			...
			} else if err := tx.Table("quota_data").Create(quotaData).Error; err != nil {
				return err
			}
		}
		return nil
	...
	lastQuotaDataExportAt.Store(time.Now().Unix())
...
func GetQuotaDataGroupByUser(startTime int64, endTime int64, userScope DataScope) (quotaData []*QuotaData, err error) {
	var quotaDatas []*QuotaData
	query := ApplyIDScope(DB.Table("quota_data"), "user_id", userScope).
...
func GetAllQuotaDates(startTime int64, endTime int64, username string, userScope DataScope) (quotaData []*QuotaData, err error) {
	if username != "" {
		var quotaDatas []*QuotaData
		query := ApplyIDScope(DB.Table("quota_data"), "user_id", userScope).
			Select("user_id, username, model_name, created_at, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used").
